...
int main()
{
  ...
  while (dataLayerClient->isConnected())
  {
    ...
    auto cpuUtilisationPercentAddress = "sdk/cpp/datalayer/provider/simple/string-Drive";
    ...
    {
      if (cpuUtilisationPercentValue.getType() == comm::datalayer::VariantType::STRING)
      {
        const char* valueAsString = cpuUtilisationPercentValue;
        std::cout << "INFO Value of " << cpuUtilisationPercentAddress << ": " << valueAsString  << " %" << std::endl;
      }
      else
      {
    ...
    auto stateAddress = "sdk/cpp/datalayer/provider/simple/string-Drive";
